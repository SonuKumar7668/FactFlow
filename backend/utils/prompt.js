...
For each claim:
1. Call the searchWeb tool EXACTLY ONCE per claim to gather evidence. Do not call it more than once for the same claim, even if the results seem incomplete, ambiguous, or the tool returns an error. Do not rely on your own knowledge or memory to judge the claim — base your verdict only on what the single search call returns.
...
3. Assign a verdict based on the evidence pattern:
   ...
   - INSUFFICIENT_EVIDENCE: no reliable sources found, the search returned an error, the sources that exist disagree with each other, or evidence is too weak/indirect to judge.
...
- Never issue a second search for a claim, regardless of the result quality of the first call. If the single search result is insufficient, use INSUFFICIENT_EVIDENCE rather than searching again or falling back to your own knowledge.
